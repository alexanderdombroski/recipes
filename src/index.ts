let currentReviever: ApiReciever;
let selectionList: MultiSelect[] = [
...
function clear() {
    ...
    const feild = document.getElementById("query") as HTMLInputElement;
    feild.value = "";
}
...
async function query() {
    ...
    const getValue = (id: string): string => {
        const input = filterSection?.querySelector(id) as HTMLInputElement;
        return input?.value ?? ""
    }
    currentReviever = new ApiReciever(
        getValue("#query"),
        selectionList[0].selection,
        selectionList[1].selection,
        selectionList[2].selection,
        selectionList[3].selection
    );
...
async function updateRecipes(queryType: Function) {
    const data = await queryType();
    if (data) {
        const grid = document.querySelector(".recipes-grid");
        grid!.innerHTML = "";
        
        const recipes = data["hits"] as {[key: string]: any}[]
...
function expandCard(event: MouseEvent) {
